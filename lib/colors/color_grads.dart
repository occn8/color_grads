import 'package:flutter/material.dart';

const List<Color> cool_blues = [
  Color(0xff2193b0),
  Color(0xff6dd5ed),
];

const List<Color> moonlit_Astroid = [
  Color(0xff0f2027),
  Color(0xff203a43),
  Color(0xff2c5364),
];

const List<Color> jshine = [
  Color(0xff12c2e9),
  Color(0xffc471ed),
  Color(0xfff64f59),
];

const List<Color> cool_sky = [
  Color(0xff2980b9),
  Color(0xff6dd5fa),
  Color(0xffffffff),
];

const List<Color> memariani = [
  Color(0xffaa4b6b),
  Color(0xff6b6b83),
  Color(0xff3b8d99),
];

const List<Color> harvey = [
  Color(0xff1f4037),
  Color(0xff99f2c8),
];

const List<Color> azur_lane = [
  Color(0xff7f7fd5),
  Color(0xff86a8e7),
  Color(0xff91eae4),
];

const List<Color> metapolis = [
  Color(0xff659999),
  Color(0xfff4791f),
];

const List<Color> kye_meh = [
  Color(0xff8360c3),
  Color(0xff2ebf91),
];

const List<Color> by_design = [
  Color(0xff009fff),
  Color(0xffec2f4b),
];

const List<Color> summer_dog = [
  Color(0xffa8ff78),
  Color(0xff78ffd6),
];

const List<Color> blue_raspberry = [
  Color(0xff00b4db),
  Color(0xff0083b0),
];

const List<Color> evening_night = [
  Color(0xff005aa7),
  Color(0xfffffde4),
];

const List<Color> red_sunset = [
  Color(0xff355c7d),
  Color(0xff6c5b7b),
  Color(0xffc06c84),
];

const List<Color> wedding_day_blues = [
  Color(0xff40e0d0),
  Color(0xffff8c00),
  Color(0xffff0080),
];

const List<Color> que_pal = [
  Color(0xff11998e),
  Color(0xff38ef7d),
];

const List<Color> pun_yeta = [
  Color(0xff108dc7),
  Color(0xffef8e38),
];

const List<Color> ohhappiness = [
  Color(0xff00b09b),
  Color(0xff96c93d),
];

const List<Color> digital_water = [
  Color(0xff74ebd5),
  Color(0xffacb6e5),
];

const List<Color> hydrogen = [
  Color(0xff667db6),
  Color(0xff0082c8),
  Color(0xff0082c8),
  Color(0xff667db6),
];

const List<Color> summer = [
  Color(0xff22c1c3),
  Color(0xfffdbb2d),
];

const List<Color> subu = [
  Color(0xff0cebeb),
  Color(0xff20e3b2),
  Color(0xff29ffc6),
];

const List<Color> socialive = [
  Color(0xff06beb6),
  Color(0xff48b1bf),
];

const List<Color> telegram = [
  Color(0xff1c92d2),
  Color(0xfff2fcfe),
];

const List<Color> scooter = [
  Color(0xff36d1dc),
  Color(0xff5b86e5),
];

const List<Color> crystal_clear = [
  Color(0xff159957),
  Color(0xff155799),
];

const List<Color> visionof_grandeur = [
  Color(0xff000046),
  Color(0xff1cb5e0),
];

const List<Color> chitty_chitty_bang_bang = [
  Color(0xff007991),
  Color(0xff78ffd6),
];

const List<Color> blue_skies = [
  Color(0xff56ccf2),
  Color(0xff2f80ed),
];

const List<Color> cinnamint = [
  Color(0xff4ac29a),
  Color(0xffbdfff3),
];

const List<Color> maldives = [
  Color(0xffb2fefa),
  Color(0xff0ed2f7),
];

const List<Color> mini = [
  Color(0xff30e8bf),
  Color(0xffff8235),
];
//star
const List<Color> pacific_dream = [
  Color(0xff34e89e),
  Color(0xff0f3443),
];
//star
const List<Color> orca = [
  Color(0xff44a08d),
  Color(0xff093637),
];

const List<Color> the_blue_lagoon = [
  Color(0xff43c6ac),
  Color(0xff191654),
];
//star
const List<Color> under_the_lake = [
  Color(0xff093028),
  Color(0xff237a57),
];

const List<Color> honey_dew = [
  Color(0xff43c6ac),
  Color(0xfff8ffae),
];

const List<Color> cocoaa_ice = [
  Color(0xffc0c0aa),
  Color(0xff1cefff),
];

const List<Color> jodhpur = [
  Color(0xff9cecfb),
  Color(0xff65c7f7),
  Color(0xff0052d4),
];

const List<Color> vice_city = [
  Color(0xff3494e6),
  Color(0xffec6ead),
];

const List<Color> mild = [
  Color(0xff67b26f),
  Color(0xff4ca2cd),
];

const List<Color> brady_brady_fun_fun = [
  Color(0xff00c3ff),
  Color(0xffffff1c),
];

const List<Color> azure_pop = [
  Color(0xffef32d9),
  Color(0xff89fffd),
];

const List<Color> disco = [
  Color(0xff4ecdc4),
  Color(0xff556270),
];

const List<Color> dania = [
  Color(0xffbe93c5),
  Color(0xff7bc6cc),
];

const List<Color> jupiter = [
  Color(0xffffd89b),
  Color(0xff19547b),
];

const List<Color> iii_delhi = [
  Color(0xff808080),
  Color(0xff3fada8),
];

const List<Color> frost = [
  Color(0xff000428),
  Color(0xff004e92),
];

const List<Color> deep_sea_space = [
  Color(0xff2c3e50),
  Color(0xff4ca1af),
];

const List<Color> sunset = [
  Color(0xff0b486b),
  Color(0xfff56217),
];

const List<Color> solid_vault = [
  Color(0xff3a7bd5),
  Color(0xff3a6073),
];

const List<Color> bright_vault = [
  Color(0xff00d2ff),
  Color(0xff928dab),
];

const List<Color> politics = [
  Color(0xff2196f3),
  Color(0xfff44336),
];

const List<Color> transfile = [
  Color(0xff16bffd),
  Color(0xffcb3066),
];
// star
const List<Color> red_ocean = [
  Color(0xff1d4350),
  Color(0xffa43931),
];

const List<Color> ali = [
  Color(0xffff4b1f),
  Color(0xff1fddff),
];

const List<Color> decent = [
  Color(0xff4ca1af),
  Color(0xffc4e0e5),
];

const List<Color> dark_skies = [
  Color(0xff4b79a1),
  Color(0xff283e51),
];

const List<Color> superman = [
  Color(0xff0099f7),
  Color(0xfff11712),
];

const List<Color> nighthawk = [
  Color(0xff2980b9),
  Color(0xff2c3e50),
];

const List<Color> miami_dolphins = [
  Color(0xff4da0b0),
  Color(0xffd39d38),
];

const List<Color> pizelex = [
  Color(0xff114357),
  Color(0xfff29492),
];

const List<Color> inbox = [
  Color(0xff457fca),
  Color(0xff5691c8),
];

const List<Color> green_and_blue = [
  Color(0xffc2e59c),
  Color(0xff64b3f4),
];

const List<Color> back_to_earth = [
  Color(0xff00c9ff),
  Color(0xff92fe9d),
];

const List<Color> clear_sky = [
  Color(0xff005c97),
  Color(0xff363795),
];

const List<Color> timber = [
  Color(0xfffc00ff),
  Color(0xff00dbde),
];

const List<Color> between_night_and_day = [
  Color(0xff2c3e50),
  Color(0xff3498db),
];

const List<Color> lizard = [
  Color(0xff304352),
  Color(0xffd7d2cc),
];

const List<Color> fresh_turboscent = [
  Color(0xfff1f2b5),
  Color(0xff135058),
];

const List<Color> turquoise_flow = [
  Color(0xff136a8a),
  Color(0xff267871),
];

const List<Color> vine = [
  Color(0xff00bf8f),
  Color(0xff001510),
];

const List<Color> atlas = [
  Color(0xfffeac5e),
  Color(0xffc779d0),
  Color(0xff4bc0c8),
];

const List<Color> endless_river = [
  Color(0xff43cea2),
  Color(0xff185a9d),
];

const List<Color> purple_bliss = [
  Color(0xff360033),
  Color(0xff0b8793),
];

const List<Color> between_the_clouds = [
  Color(0xff73c8a9),
  Color(0xff373b44),
];

const List<Color> facebook_messeger = [
  Color(0xff00c6ff),
  Color(0xff0072ff),
];

const List<Color> shore = [
  Color(0xff70e1f5),
  Color(0xffffd194),
];

const List<Color> neon_life = [
  Color(0xffb3ffab),
  Color(0xff12fff7),
];

const List<Color> teal_love = [
  Color(0xffaaffa9),
  Color(0xff11ffbd),
];

const List<Color> reef = [
  Color(0xff00d2ff),
  Color(0xff3a7bd5),
];

const List<Color> winter = [
  Color(0xffe6dada),
  Color(0xff274046),
];

const List<Color> forever_lost = [
  Color(0xff5d4157),
  Color(0xffa8caba),
];

const List<Color> moor = [
  Color(0xff616161),
  Color(0xff9bc5c3),
];

const List<Color> aqulicious = [
  Color(0xff50c9c3),
  Color(0xff96deda),
];

const List<Color> miaka = [
  Color(0xfffc354c),
  Color(0xff0abfbc),
];

const List<Color> calm_darya = [
  Color(0xff5f2c82),
  Color(0xff49a09d),
];

const List<Color> stellar = [
  Color(0xff7474bf),
  Color(0xff348ac7),
];

const List<Color> mantle = [
  Color(0xff24c6dc),
  Color(0xff514a9d),
];

const List<Color> opa = [
  Color(0xff3d7eaa),
  Color(0xffffe47a),
];

const List<Color> sea_blizz = [
  Color(0xff1cd8d2),
  Color(0xff93edc7),
];

const List<Color> shroom_haze = [
  Color(0xff5c258d),
  Color(0xff4389a2),
];

const List<Color> moos = [
  Color(0xff134e5e),
  Color(0xff71b280),
];

const List<Color> bora_bora = [
  Color(0xff2bc0e4),
  Color(0xffeaecc6),
];

const List<Color> venice_blue = [
  Color(0xff085078),
  Color(0xff85d8ce),
];

const List<Color> mirage = [
  Color(0xff16222a),
  Color(0xff3a6073),
];

const List<Color> mojito = [
  Color(0xff1d976c),
  Color(0xff93f9b9),
];

const List<Color> sea_weed = [
  Color(0xff4cb8c4),
  Color(0xff3cd3ad),
];

const List<Color> stripe = [
  Color(0xff1fa2ff),
  Color(0xff12d8fa),
  Color(0xffa6ffcb),
];

const List<Color> aqua_marine = [
  Color(0xff1a2980),
  Color(0xff26d0ce),
];

const List<Color> rose_water = [
  Color(0xffe55d87),
  Color(0xff5fc3e4),
];

const List<Color> emerald_water = [
  Color(0xff348f50),
  Color(0xff56b4d3),
];

const List<Color> green_beach = [
  Color(0xff02aab0),
  Color(0xff00cdac),
];

const List<Color> harmonic_energy = [
  Color(0xff16a085),
  Color(0xfff4d03f),
];

const List<Color> hazel = [
  Color(0xff77a1d3),
  Color(0xff79cbca),
  Color(0xffe684ae),
];

const List<Color> nimvelo = [
  Color(0xff314755),
  Color(0xff26a0da),
];

const List<Color> sea_blue = [
  Color(0xff2b5876),
  Color(0xff4e4376),
];

const List<Color> sexy_blue = [
  Color(0xff2193b0),
  Color(0xff6dd5ed),
];

const List<Color> skyline = [
  Color(0xff1488cc),
  Color(0xff2b32b2),
];

const List<Color> sel = [
  Color(0xff00467f),
  Color(0xffa5cc82),
];

const List<Color> sky = [
  Color(0xff076585),
  Color(0xffffffff),
];

const List<Color> windy = [
  Color(0xffacb6e5),
  Color(0xff86fde8),
];

const List<Color> bupe = [
  Color(0xff00416a),
  Color(0xffe4e5e6),
];

const List<Color> lunada = [
  Color(0xff5433ff),
  Color(0xff20bdff),
  Color(0xffa5fecb),
];

const List<Color> anwar = [
  Color(0xff334d50),
  Color(0xffcbcaa5),
];

const List<Color> combi = [
  Color(0xff00416a),
  Color(0xff799f0c),
  Color(0xffffe000),
];

const List<Color> blu = [
  Color(0xff00416a),
  Color(0xffe4e5e6),
];

const List<Color> piggy_pink = [
  Color(0xffee9ca7),
  Color(0xffffdde1),
];

const List<Color> megatron = [
  Color(0xffc6ffdd),
  Color(0xfffbd786),
  Color(0xfff7797d),
];

const List<Color> evening_sunshine = [
  Color(0xffb92b27),
  Color(0xff1565c0),
];

const List<Color> watching_hour = [
  Color(0xffc31432),
  Color(0xff240b36),
];

const List<Color> flare = [
  Color(0xfff12711),
  Color(0xfff5af19),
];

const List<Color> kyoo_pal = [
  Color(0xffdd3e54),
  Color(0xff6be585),
];

const List<Color> ultra_violet = [
  Color(0xff654ea3),
  Color(0xffeaafc8),
];

const List<Color> burning_orange = [
  Color(0xffff416c),
  Color(0xffff4b2b),
];

const List<Color> wiretap = [
  Color(0xff8a2387),
  Color(0xffe94057),
  Color(0xfff27121),
];

const List<Color> rastafari = [
  Color(0xff1e9600),
  Color(0xfffff200),
  Color(0xffff0000),
];
//star
const List<Color> sin_city_red = [
  Color(0xffed213a),
  Color(0xff93291e),
];
//star
const List<Color> citrus_peel = [
  Color(0xfffdc830),
  Color(0xfff37335),
];
//star
const List<Color> venusa = [
  Color(0xffda4453),
  Color(0xff89216b),
];

const List<Color> pure_lust = [
  Color(0xff333333),
  Color(0xffdd1818),
];
//star
const List<Color> shifter = [
  Color(0xffbc4e9c),
  Color(0xfff80759),
];

const List<Color> sublime_light = [
  Color(0xfffc5c7d),
  Color(0xff6a82fb),
];

const List<Color> sublime_vivid = [
  Color(0xfffc466b),
  Color(0xff3f5efb),
];

const List<Color> bighead = [
  Color(0xffc94b4b),
  Color(0xff4b134f),
];

const List<Color> taran_tado = [
  Color(0xff23074d),
  Color(0xffcc5333),
];

const List<Color> relaxing_red = [
  Color(0xfffffbd5),
  Color(0xffb20a2c),
];
//star
const List<Color> orange_fun = [
  Color(0xfffc4a1a),
  Color(0xfff7b733),
];

const List<Color> velvet_sun = [
  Color(0xffe1eec3),
  Color(0xfff05053),
];

const List<Color> king_yna = [
  Color(0xff1a2a6c),
  Color(0xffb21f1f),
  Color(0xfffdbb2d),
];
//star
const List<Color> orange_coral = [
  Color(0xffff9966),
  Color(0xffff5e62),
];

const List<Color> crimson_tide = [
  Color(0xff642b73),
  Color(0xffc6426e),
];
//star
const List<Color> alive = [
  Color(0xffcb356b),
  Color(0xffbd3f32),
];

const List<Color> relay = [
  Color(0xff3a1c71),
  Color(0xffd76d77),
  Color(0xffffaf7b),
];

const List<Color> compare_now = [
  Color(0xffef3b36),
  Color(0xffffffff),
];

const List<Color> mello = [
  Color(0xffc0392b),
  Color(0xff8e44ad),
];

const List<Color> sunkist = [
  Color(0xfff2994a),
  Color(0xfff2c94c),
];

const List<Color> coal = [
  Color(0xffeb5757),
  Color(0xff000000),
];

const List<Color> html = [
  Color(0xffe44d26),
  Color(0xfff16529),
];

const List<Color> sha_la_la = [
  Color(0xffd66d75),
  Color(0xffe29587),
];

const List<Color> celestial = [
  Color(0xffc33764),
  Color(0xff1d2671),
];

const List<Color> love_and_liberty = [
  Color(0xff200122),
  Color(0xff6f0000),
];
//star
const List<Color> roseanna = [
  Color(0xffffafbd),
  Color(0xffffc3a0),
];

const List<Color> rose_colored_lenses = [
  Color(0xffe8cbc0),
  Color(0xff636fa4),
];

const List<Color> jaipur = [
  Color(0xffdbe6f6),
  Color(0xffc5796d),
];
//star
const List<Color> dawn = [
  Color(0xfff3904f),
  Color(0xff3b4371),
];
//star
const List<Color> ibiza_sunset = [
  Color(0xffee0979),
  Color(0xffff6a00),
];

const List<Color> radar = [
  Color(0xffa770ef),
  Color(0xffcf8bf3),
  Color(0xfffdb99b),
];
//star
const List<Color> eds_sunset_gradient = [
  Color(0xffff7e5f),
  Color(0xfffeb47b),
];

const List<Color> nepal = [
  Color(0xffde6161),
  Color(0xff2657eb),
];

const List<Color> love_couple = [
  Color(0xff3a6186),
  Color(0xff89253e),
];
//star
const List<Color> blood_red = [
  Color(0xfff85032),
  Color(0xffe73827),
];
//star
const List<Color> firewatch = [
  Color(0xffcb2d3e),
  Color(0xffef473a),
];

const List<Color> minimal_red = [
  Color(0xfff00000),
  Color(0xffdc281e),
];

const List<Color> dusk = [
  Color(0xff2c3e50),
  Color(0xfffd746c),
];

const List<Color> grapefruit_sunset = [
  Color(0xffe96443),
  Color(0xff904e95),
];

const List<Color> sweet_morning = [
  Color(0xffff5f6d),
  Color(0xffffc371),
];
//star
const List<Color> sylvia = [
  Color(0xffff4b1f),
  Color(0xffff9068),
];

const List<Color> tranquil = [
  Color(0xffeecda3),
  Color(0xffef629f),
];

const List<Color> purple_white = [
  Color(0xffba5370),
  Color(0xfff4e2d8),
];

const List<Color> forest = [
  Color(0xff5a3f37),
  Color(0xff2c7744),
];

const List<Color> christmas = [
  Color(0xff2f7336),
  Color(0xffaa3a38),
];

const List<Color> haikus = [
  Color(0xfffd746c),
  Color(0xffff9068),
];

const List<Color> blush = [
  Color(0xffb24592),
  Color(0xfff15f79),
];

const List<Color> back_to_the_future = [
  Color(0xffc02425),
  Color(0xfff0cb35),
];

const List<Color> poncho = [
  Color(0xff403a3e),
  Color(0xffbe5869),
];

const List<Color> netflix = [
  Color(0xff8e0e00),
  Color(0xff1f1c18),
];
// star
const List<Color> master_card = [
  Color(0xfff46b45),
  Color(0xffeea849),
];
//star
const List<Color> passion = [
  Color(0xffe53935),
  Color(0xffe35d5b),
];

const List<Color> piglet = [
  Color(0xffee9ca7),
  Color(0xffffdde1),
];

const List<Color> virgin_america = [
  Color(0xff7b4397),
  Color(0xffdc2430),
];

const List<Color> instagram = [
  Color(0xff833ab4),
  Color(0xfffd1d1d),
  Color(0xfffcb045),
];

const List<Color> predawn = [
  Color(0xffffa17f),
  Color(0xff00223e),
];
//star
const List<Color> soundcloud = [
  Color(0xfffe8c00),
  Color(0xfff83600),
];

const List<Color> chear_up_emo_kid = [
  Color(0xff556270),
  Color(0xffff6b6b),
];

const List<Color> man_of_steel = [
  Color(0xff780206),
  Color(0xff061161),
];
//star
const List<Color> red_mist = [
  Color(0xff000000),
  Color(0xffe74c3c),
];
//star
const List<Color> dance_to_forget = [
  Color(0xffff4e50),
  Color(0xfff9d423),
];

const List<Color> virgin = [
  Color(0xffc9ffbf),
  Color(0xffffafbd),
];
//star
const List<Color> the_strain = [
  Color(0xff870000),
  Color(0xff190a05),
];

const List<Color> candy = [
  Color(0xffd3959b),
  Color(0xffbfe6ba),
];
//star
const List<Color> nelson = [
  Color(0xfff2709c),
  Color(0xffff9472),
];

const List<Color> almost = [
  Color(0xffddd6f3),
  Color(0xfffaaca8),
];
//star
const List<Color> kyoto = [
  Color(0xffc21500),
  Color(0xffffc500),
];

const List<Color> a_lost_memory = [
  Color(0xffde6262),
  Color(0xffffb88c),
];
//star
const List<Color> blurry_beach = [
  Color(0xffd53369),
  Color(0xffcbad6d),
];
//star
const List<Color> namn = [
  Color(0xffa73737),
  Color(0xff7a2828),
];

const List<Color> day_tripper = [
  Color(0xfff857a6),
  Color(0xffff5858),
];
//star
const List<Color> shrimpy = [
  Color(0xffe43a15),
  Color(0xffe65245),
];

const List<Color> influenza = [
  Color(0xffc04848),
  Color(0xff480048),
];
//star
const List<Color> bourbon = [
  Color(0xffec6f66),
  Color(0xfff3a183),
];

const List<Color> moonrise = [
  Color(0xffdae2f8),
  Color(0xffd6a4a4),
];

const List<Color> peach = [
  Color(0xffed4264),
  Color(0xffffedbc),
];

const List<Color> dracula = [
  Color(0xffdc2424),
  Color(0xff4a569d),
];
//star
const List<Color> cherry = [
  Color(0xffeb3349),
  Color(0xfff45c43),
];

const List<Color> pinky = [
  Color(0xffdd5e89),
  Color(0xfff7bb97),
];
//star
const List<Color> sunrise = [
  Color(0xffff512f),
  Color(0xfff09819),
];
//star
const List<Color> bloody_mary = [
  Color(0xffff512f),
  Color(0xffdd2476),
];
//star
const List<Color> playing_with_reds = [
  Color(0xffd31027),
  Color(0xffea384d),
];

const List<Color> cool_brown = [
  Color(0xff603813),
  Color(0xffb29f94),
];

const List<Color> youtube = [
  Color(0xffe52d27),
  Color(0xffb31217),
];

const List<Color> noon_to_dusk = [
  Color(0xffff6e7f),
  Color(0xffbfe9ff),
];
//star
const List<Color> blooker20 = [
  Color(0xffe65c00),
  Color(0xfff9d423),
];

const List<Color> purple_love = [
  Color(0xffcc2b5e),
  Color(0xff753a88),
];
//star 
const List<Color> dimigo = [
  Color(0xffec008c),
  Color(0xfffc6767),
];

const List<Color> cooper = [
  Color(0xffb79891),
  Color(0xff94716b),
];

const List<Color> kyoo_tah = [
  Color(0xff544a7d),
  Color(0xffffd452),
];

const List<Color> sand_to_blue = [
  Color(0xff3e5151),
  Color(0xffdecba4),
];
//star
const List<Color> selenium = [
  Color(0xff3c3b3f),
  Color(0xff605c3c),
];

const List<Color> sulphur = [
  Color(0xffcac531),
  Color(0xfff3f9a7),
];
//star
const List<Color> lithuim = [
  Color(0xff6d6027),
  Color(0xffd3cbb8),
];
//star
const List<Color> learning_and_leading = [
  Color(0xfff7971e),
  Color(0xffffd200),
];

const List<Color> sun_on_the_horizon = [
  Color(0xfffceabb),
  Color(0xfff8b500),
];

const List<Color> sherbert = [
  Color(0xfff79d00),
  Color(0xff64f38c),
];

const List<Color> minnesota_vikings = [
  Color(0xff5614b0),
  Color(0xffdbd65c),
];

const List<Color> pale_wood = [
  Color(0xffeacda3),
  Color(0xffd6ae7b),
];

const List<Color> light_orange = [
  Color(0xffffb75e),
  Color(0xffed8f03),
];

const List<Color> dark_knight = [
  Color(0xffba8b02),
  Color(0xff181818),
];

const List<Color> ukraine = [
  Color(0xff004ff9),
  Color(0xfffff94c),
];

const List<Color> koko_caramel = [
  Color(0xffd1913c),
  Color(0xffffd194),
];

const List<Color> pastel_orange_at_the_sun = [
  Color(0xffffb347),
  Color(0xffffcc33),
];
//star
const List<Color> crazy_orange_1 = [
  Color(0xffd38312),
  Color(0xffa83279),
];

const List<Color> starfall = [
  Color(0xfff0c27b),
  Color(0xff4b1248),
];

const List<Color> earthly = [
  Color(0xff649173),
  Color(0xffdbd5a4),
];

const List<Color> autumn = [
  Color(0xffdad299),
  Color(0xffb0dab9),
];

const List<Color> vasily = [
  Color(0xffe9d362),
  Color(0xff333333),
];

const List<Color> mango_pulp = [
  Color(0xfff09819),
  Color(0xffedde5d),
];

const List<Color> juicy_orange = [
  Color(0xffff8008),
  Color(0xffffc837),
];

const List<Color> monte_carlo = [
  Color(0xffcc95c0),
  Color(0xffdbd4b4),
  Color(0xff7aa1d2),
];

const List<Color> lemon_twist = [
  Color(0xff3ca55c),
  Color(0xffb5ac49),
];

const List<Color> sunny_days = [
  Color(0xffede574),
  Color(0xffe1f5c4),
];

const List<Color> rea = [
  Color(0xffffe000),
  Color(0xff799f0c),
];

const List<Color> mango = [
  Color(0xffffe259),
  Color(0xffffa751),
];

const List<Color> ver = [
  Color(0xffffe000),
  Color(0xff799f0c),
];
/////////////
///
const List<Color> metalic_toad = [
  Color(0xffabbaab),
  Color(0xffffffff),
];

const List<Color> servquick = [
  Color(0xff485563),
  Color(0xff29323c),
];
const List<Color> cheer_up_emo_kid = [
  Color(0xff556270),
  Color(0xffff6b6b),
];
const List<Color> ash = [
  Color(0xff606c88),
  Color(0xff3f4c6b),
];

const List<Color> titanium = [
  Color(0xff283048),
  Color(0xff859398),
];
const List<Color> mystic = [
  Color(0xff757f9a),
  Color(0xffd7dde8),
];
const List<Color> copper = [
  Color(0xffb79891),
  Color(0xff94716b),
];
const List<Color> royal_blue = [
  Color(0xff536976),
  Color(0xff292e49),
];
const List<Color> margo = [
  Color(0xffffefba),
  Color(0xffffffff),
];
const List<Color> delicate = [
  Color(0xffd3cce3),
  Color(0xffe9e4f0),
];
const List<Color> pink_flavour = [
  Color(0xff800080),
  Color(0xffffc0cb),
];
const List<Color> dull = [
  Color(0xffc9d6ff),
  Color(0xffe2e2e2),
];
const List<Color> broken_hearts = [
  Color(0xffd9a7c7),
  Color(0xfffffcdc),
];
const List<Color> black_rose = [
  Color(0xfff4c4f3),
  Color(0xfffc67fa),
];
const List<Color> colors_of_sky = [
  Color(0xffe0eafc),
  Color(0xffcfdef3),
];
const List<Color> friday = [
  Color(0xff83a4d4),
  Color(0xffb6fbff),
];
const List<Color> cherryblossoms = [
  Color(0xfffbd3e9),
  Color(0xffbb377d),
];
const List<Color> clouds = [
  Color(0xffece9e6),
  Color(0xffffffff),
];
const List<Color> purple_paradise = [
  Color(0xff1d2b64),
  Color(0xfff8cdda),
];
const List<Color> anamnisar = [
  Color(0xff9796f0),
  Color(0xfffbc7d4),
];
const List<Color> amin = [
  Color(0xff8e2de2),
  Color(0xff4a00e0),
];
const List<Color> neuromancer = [
  Color(0xfff953c6),
  Color(0xffb91d73),
];
const List<Color> expresso = [
  Color(0xffad5389),
  Color(0xff3c1053),
];
const List<Color> purpink = [
  Color(0xff7f00ff),
  Color(0xffe100ff),
];
const List<Color> can_you_feel_the_love_toight = [
  Color(0xff4568dc),
  Color(0xffb06ab3),
];
const List<Color> cosmic_fusion = [
  Color(0xffff00cc),
  Color(0xff333399),
];
const List<Color> mauve = [
  Color(0xff42275a),
  Color(0xff42275a),
];
const List<Color> suzy = [
  Color(0xff834d9b),
  Color(0xffd04ed6),
];
const List<Color> purplin = [
  Color(0xff6a3093),
  Color(0xffa044ff),
];

const List<Color> amethyst = [
  Color(0xff9d50bb),
  Color(0xff6e48aa),
];
const List<Color> dirty_fog = [
  Color(0xffb993d6),
  Color(0xff8ca6db),
];
const List<Color> aubergine = [
  Color(0xffaa076b),
  Color(0xff61045f),
];
const List<Color> intuitive_purple = [
  Color(0xffda22ff),
  Color(0xff9733ee),
];
const List<Color> slight_ocean_view = [
  Color(0xffa8c0ff),
  Color(0xff3f2b96),
];
const List<Color> moon_purple = [
  Color(0xff4e54c8),
  Color(0xff8f94fb),
];
const List<Color> kimoby_is_the_new_blue = [
  Color(0xff396afc),
  Color(0xff2948ff),
];
const List<Color> venice = [
  Color(0xff6190e8),
  Color(0xffa7bfe8),
];
const List<Color> very_blue = [
  Color(0xff0575e6),
  Color(0xff021b79),
];
const List<Color>  joomla= [
  Color(0xff1e3c72),
  Color(0xff2a5298),
];
const List<Color> deep_purple = [
  Color(0xff673ab7),
  Color(0xff512da8),
];
const List<Color> electric_violet = [
  Color(0xff4776e6),
  Color(0xff8e54e9),
];
const List<Color> kashmir = [
  Color(0xff614385),
  Color(0xff516395),
];
const List<Color> bluelagoo = [
  Color(0xff0052d4),
  Color(0xff4364f7),
  Color(0xff6fb1fc),
];